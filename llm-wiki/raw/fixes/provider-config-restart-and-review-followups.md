...
During local Vite validation with an isolated `CODEX_HOME`, the app started without auth and selected OpenCode Zen / `big-pickle`. After copying a valid host `auth.json` into the same `CODEX_HOME`, Settings showed `Codex`, but the new-chat composer still showed `big-pickle`.
...
- Browser validation on the local Vite URL showed the new-chat model changed from `big-pickle` to a concrete Codex model.
...
A project-scoped new chat using a concrete Codex model successfully created a thread and rendered the assistant reply `Hi`.
