...
| 2026-05-28 | Phase 7 | Any-site injection hotfix | Completed | Parallel worker/reviewer cycle added runtime optional host access for normal `http(s)` pages. The extension now ships narrow permanent pairing host permissions plus `optional_host_permissions` for `http://*/*` and `https://*/*`; the side panel requests current-site access directly inside the `Inject overlay` click path, while the service worker no longer calls `chrome.permissions.request`. The final package SHA-256 is `015c3a450b5f3fbe2fe59d1800ee672fc173e33850fba369315bc6c93b488c8c`. Verification: manifest validation, pairing smoke, service-worker persistence smoke, package build, production artifact validation, and reviewer re-check passed. Manual Chrome approval prompt remains the final acceptance check. |
| 2026-05-28 | Phase 7 | Compact listener and revoked sync | Completed | Parallel worker/reviewer cycle made the Codex UI listener compact by default with setup/token details hidden behind disclosure. Backend listen status now returns `status: revoked` for the same authorized token/session after extension send/stop, allowing Codex UI polling to stop showing an active listener instead of treating the state as a generic expired token. Verification: `pnpm exec vue-tsc --noEmit` passed for UI worker; `pnpm exec vitest run src/server/browserAnnotationListen.test.ts --reporter=verbose` passed 8 tests for revoked-status sync. |
