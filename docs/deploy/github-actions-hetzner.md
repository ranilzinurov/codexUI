...
The deploy script syncs static assets to `/var/www/codexui-dist` by default.
Make that directory writable by the deploy user instead of granting broad sudo:
...
4. Hetzner runs `scripts/deploy-from-github.sh`, fetches `origin/main`, resets
   ...
   Tailwind Oxide are present, rebuilds, syncs `dist/` to
   `/var/www/codexui-dist`, restarts `codexui`, and waits for a healthcheck.
