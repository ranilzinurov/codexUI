...
DevTools capture uses Chrome's debugger permission only after the user enables **DevTools capture mode** in the side panel. Capture stops on explicit disable, successful send, tab close, timeout, or debugger detach. Console and network rows are count- and byte-bounded; request and response bodies remain metadata-only until the explicit body-capture stage.
