...
- `manifest.json` declares MV3, the service worker, side panel, action keyboard shortcut, `activeTab`/`alarms`/`debugger`/`scripting`/`tabs`/`sidePanel` permissions, target host access for `https://annotate.todo-tg-app.ru/*`, and narrow local development host access for `http://127.0.0.1/*` plus `http://localhost/*`.
