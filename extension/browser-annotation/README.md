...
Manifest V3 MVP scaffold for browser annotation. This stage intentionally stops at extension structure, settings placeholders, connected/disconnected placeholder state, restricted-page guards, and user-requested overlay injection.

No build step is required. The folder is designed to be loaded directly with Chrome's **Load unpacked** flow.

## Files

- `manifest.json` declares MV3, the service worker, side panel, `activeTab`/`scripting`/`tabs`/`sidePanel` permissions, and target host access for `https://annotate.todo-tg-app.ru/*`.
- `service-worker/service-worker.js` owns side-panel messages, local settings, active-tab checks, action-click side-panel behavior, and user-gesture content-script injection.
...
- `shared/` contains small globals, JSDoc contract typedefs, message names, storage keys, URL rules, and defaults used by the service worker, content script, and side panel.
...
6. Save placeholder settings if desired, then click **Inject overlay**.
7. Confirm the page shows the "Codex annotation mode" overlay placeholder.
