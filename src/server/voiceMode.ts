...
const VOICE_PROFILES: Record<VoiceProfile, VoiceProfileConfig> = {
  economy: {
    ...
    maxOutputTokens: 320,
    reasoningEffort: 'low',
    guidance: 'Очень коротко: только итог, риск и что проверить. Примерно 20-40 секунд речи.',
  },
  medium: {
    maxChars: 1_400,
    maxOutputTokens: 650,
    reasoningEffort: 'low',
    guidance: 'Нормально подробно, но без лишних деталей. Примерно 1-2 минуты речи.',
  },
  forte: {
    maxChars: 2_400,
    maxOutputTokens: 1_000,
    reasoningEffort: 'medium',
    guidance: 'Можно подробнее объяснить ход и важные нюансы, но все равно не читать код, диффы или логи.',
  },
}
...
function buildVoiceSummaryInstructions(profile: VoiceProfile): string {
  const profileConfig = VOICE_PROFILES[profile]
  return [
    'Ты готовишь русскую голосовую версию ответа Codex для пользователя, который идет по улице в наушниках.',
    'Всегда отвечай по-русски, простым разговорным языком.',
    ...
    'Если исходный ответ содержит код, диффы или логи, коротко объясни, что код делает, что изменилось и что важно проверить.',
    'Скажи: что сделано, где важный риск, что проверить дальше, и была ли ошибка.',
